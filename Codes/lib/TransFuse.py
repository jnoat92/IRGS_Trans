...
class TransFuse_S(nn.Module):
    def __init__(self, img_size = 320, patch_size = 16, num_heads=6, depth=8, embed_dim=384,
                       in_chans=3, num_classes=1, cnn="resnet34",
                       drop_rate=0.2, normal_init=True, pretrained=False):
        super(TransFuse_S, self).__init__()

        self.net_name = 'Transfuse'

        # CNN branch
        cnn_func = getattr(models, cnn)
        self.cnn = cnn_func()
        ...
        self.cnn.conv1 = nn.Conv2d(in_chans, 64, kernel_size=7, stride=1, 
                                      padding=3, bias=False)
        self.cnn.inplanes = 64
        # self.cnn.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.cnn.maxpool = nn.MaxPool2d(kernel_size=1, stride=1, padding=0)
        self.cnn.layer1 = self.cnn._make_layer(BasicBlock, 64, layers[0])
        self.cnn.layer2 = self.cnn._make_layer(BasicBlock, 128, layers[1], stride=1)
        self.cnn.layer3 = self.cnn._make_layer(BasicBlock, 256, layers[2], stride=2)
        # --------------------------------------

        if pretrained:
            self.cnn.load_state_dict(torch.load('pretrained/resnet34-333f7ec4.pth'))
    ...
    def forward(self, imgs, labels=None):
        ...
        # USING t0(64x64) and discarding t1 and t2
        x_c = self.up_c(x_u, x_b)
        map_x = F.interpolate(self.final_x(x_c), scale_factor=2, mode='bilinear', align_corners=True)
        map_x = self.softmax(map_x)

        return map_x

        # USING t0(16x16), t1(32x32), and t2(64x64)
            # x_c = self.up_c(x_u, x_b)
...
class Up(nn.Module):
    ...
    def forward(self, x1, x2=None):
        ...
        if x2 is not None:
            ...
            hx = torch.div(diffX, 2, rounding_mode='trunc')
            hy = torch.div(diffY, 2, rounding_mode='trunc')
            x1 = F.pad(x1, [hx, diffX - hx,
                            hy, diffY - hy])
