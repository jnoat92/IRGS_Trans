...
class Attention(nn.Module):
    ...
    def forward(self, x, mask=None):
        B, N, C = x.shape               # (batch, N_tokens, emb_dim)

        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q, k, v = qkv[0], qkv[1], qkv[2]    # make torchscript happy (cannot use tensor as tuple)
                                            # (batch, N_heads, N_tokens, head_dim)     emb_dim = head_dims * N_heads
        
        # attn = torch.zeros_like(q)
        # for i in range(q.shape[2]):
        #     qi = q[:,:,i:i+1,:]
        #     qk_i = (qi @ k.transpose(-2, -1)) * self.scale
        #     soft_i = qk_i.softmax(dim=-1)
        #     soft_i = self.attn_drop(soft_i)
        #     attn[:,:,i:i+1,:] = soft_i @ v
        ...
        if mask is not None:
            # See https://towardsdatascience.com/how-to-code-the-transformer-in-pytorch-24db27c8f9ec#1b3f
